...
RULES:
- PREFER frappe-ui components. Always reach for a catalog component (Button, Badge, Alert, FormControl, ListView, Tabs, Dialog, etc.) before hand-building the same thing from container/div + styles. Only assemble custom markup from containers when no catalog component fits — never recreate an existing component.
